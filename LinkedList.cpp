...
void LinkedList<T>::rotate(int k){
  if(k == 0){//limits how many times the function can recurse
    return;
  }
  else{
    insert(0, getEntry(item_count_-1));//places last element as first
    remove(item_count_-1);//deletes the last element
    rotate(k - 1);//calls again
  }
}
...
template<class T>
void LinkedList<T>:: invertRest(Node<T>* curr){
  if (curr == nullptr) {
        return;
    }
    ...
    invertRest(curr->getNext());//recursive list traverse
    curr->getNext()->setNext(curr);//front elements goes back
    curr->setNext(nullptr);//now that its in back, it points to null
}
